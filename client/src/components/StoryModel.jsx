  ...
  return (
    <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur text-white flex items-center justify-center p-4'>
        <div className='w-full max-w-md'>
            ...
            <div className='rounded-lg h-96 flex items-center justify-center relative' style={{backgroundColor: background}}>
...

                {
                    mode === "media" && previewUrl && (
                        ...
                        (
                            <video src={previewUrl} className='object-contain max-h-full' alt="" />
                        )
                    )
                }
            ...
            <div className='flex mt-4 gap-2'>
                <button onClick={() => {setMode('text'); setMedia(null); setPreviewUrl(null)}} className={`flex flex-1 items-center justify-center gap-2 p-2 rounded ${mode === "text" ? "bg-white text-black" : "bg-zinc-800"} cursor-pointer`}>
